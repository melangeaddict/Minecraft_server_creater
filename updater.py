...
for version in data['versions']:
    if version['id'] == minecraft_ver:
        ...
        log('Your sha1 is ' + cur_ver + '. Latest version is ' + str(minecraft_ver) + " with sha1 of " + jarsha,
            logFile)
        ...
        if cur_ver != jarsha:
            ...
            os.system(
                'screen -S minecraft%%%% -X stuff \'say ATTENTION: Server will shutdown for 1 minute to update in 30 seconds.^M\'')
            ...
            logFile.close()
            os.chdir("..")
            os.system('screen -S minecraft%%%% -d -m java -Xmx^^^^M -Xms1024M -jar minecraft_server.jar')
